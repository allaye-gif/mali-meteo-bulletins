...
REM Verifier si la base mali_meteo_bul existe
set "TEMP_DBCHECK=%TEMP%\meteo_dbcheck.txt"
"%PSQL%" -U postgres -h localhost -p 5432 -t -c "SELECT COUNT(*) FROM pg_database WHERE datname='mali_meteo_bul'" > "%TEMP_DBCHECK%" 2>nul
set "DB_EXISTS=0"
...
if "!DB_EXISTS!"=="0" (
    echo [DB] Creation de la base mali_meteo_bul...
    "%PSQL%" -U postgres -h localhost -p 5432 -c "CREATE DATABASE mali_meteo_bul ENCODING 'UTF8' TEMPLATE=template0"
    if errorlevel 1 (
        echo [ERREUR] Impossible de creer la base mali_meteo_bul.
        goto fin
    )
    echo [OK] Base mali_meteo_bul creee
) else (
    echo [OK] Base mali_meteo_bul existante
)
...
if not exist ".env" (
    (
        echo DATABASE_URL=postgresql://postgres:allaye@localhost:5432/mali_meteo_bul
        echo PORT=1005
...
"%PSQL%" -U postgres -h localhost -p 5432 -d mali_meteo_bul -t -c "SELECT COUNT(*) FROM bulletins" > "%TEMP_COUNT%" 2>nul
...
if "!BULLETIN_COUNT!"=="0" (
    if exist "deploy\database_dump.sql" (
        ...
        "%PSQL%" -U postgres -h localhost -p 5432 -d mali_meteo_bul -c "DROP TABLE IF EXISTS bulletins, templates CASCADE" >nul 2>&1
        "%PSQL%" -U postgres -h localhost -p 5432 -d mali_meteo_bul -f "deploy\database_dump.sql" >nul 2>&1
        if not errorlevel 1 (
...
echo.
echo   Base de donnees : mali_meteo_bul @ localhost:5432
